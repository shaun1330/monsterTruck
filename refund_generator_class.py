...
class RefundGenerator:
    def __init__(self, pdf_name, invoice_total, cash_paid, transfer_paid,  amount_paid, amount_refunded, cash_or_transfer):
        ...
        self.text(0.62, 0.93, "REFUND", 30, self.font)
        boxWidth = 210
        boxHeight = 20
        ...
        self.text(0.65, 0.88, "REFUND #", 10, self.font)
        self.text(0.84, 0.88, "DATE", 10, self.font)
        self.text(0.65, 0.80, "Refund Total", 10, self.font)
        ...
        self.c.setFillColorRGB(0, 0, 0)
        self.text(0.07, 0.81, "REFUND TO", 10, self.font)
        ...
        self.right_text(0.74, 0.204, "- "*20, 12, self.font)
        # self.right_text(0.74, 0.223, ':', 12, self.font)

        self.right_text(0.74, 0.185, "Refund Amount:", 12, self.font)
        # self.right_text(0.74, 0.204, ':', 12, self.font)
        self.right_text(0.9, 0.185, f"${self.amount_refunded}", 12, self.font)

        self.right_text(0.74, 0.166, "Refund Type:", 12, self.font)
        # self.right_text(0.74, 0.204, ':', 12, self.font)
        self.right_text(0.9, 0.166, f"{cash_or_transfer}", 12, self.font)

        self.right_text(0.74, 0.147, "Balance Due:", 12, self.font)
        # self.right_text(0.74, 0.204, ':', 12, self.font)
        self.right_text(0.9, 0.147, "$0.00", 12, self.font)
...
# item_codes, desc, prices, qtys, subtotals
    def invoice_line(self, item_codes, item_descs, item_prices, qtys, subtotals):
        ...
        for i in item_codes:
            ...
            c += 1
        self.text(0.065, 0.67 - 0.03 * c, 'Refund', 12, self.font)

        c = 0
        for i in item_descs:
            self.text(0.185, 0.67-0.03*c, str(i), 12, self.font)
            c += 1
        self.text(0.185, 0.67 - 0.03 * c, '- '*10 + 'Refund' + ' -'*10, 12, self.font)
        ...
        for i in item_prices:
            ...
            c += 1
        self.right_text(0.66, 0.67 - 0.03 * c, str(self.amount_refunded*-1), 12, self.font)

        c = 0
        for i in qtys:
            self.text(0.685, 0.67 - 0.03 * c, str(i), 12, self.font)
            c += 1
        self.text(0.685, 0.67 - 0.03 * c, "-1", 12, self.font)
        ...
        for i in subtotals:
            ...
            c += 1
        self.right_text(0.9, 0.67 - 0.03 * c, str(self.amount_refunded*-1), 12, self.font)
    ...
    def refundNo(self, invoice_no):
        self.text(0.65, 0.855, str(invoice_no), 12, self.font)

    def refundDate(self, date):
...
if __name__ == "__main__":
    refund_gen = RefundGenerator(pdf_name='Test_refund.pdf',
                    invoice_total=100,
                    cash_paid=50,
                    transfer_paid=50,
                    amount_paid=100,
                    amount_refunded=100,
                    cash_or_transfer='Cash',
                    )
    refund_gen.invoice_line([1], ['Test'], [100], [1], [100])
    refund_gen.memberName('Test Test')
    refund_gen.refundNo(90000)
